...
def main(argv):
	...
	class ReplacementScanf(angr.SimProcedure):
		# Hint: scanf("%u %20s")
		def run(self, _, address_u, address_20s):
			# %u
			...
			self.state.memory.store(address_u,
			                        scanf0,
			                        endness=project.arch.memory_endness)
			self.state.memory.store(address_20s, scanf1)
	...
	def check_strncpy(state):
		...
		strncpy_dest = state.memory.load(state.regs.esp + 4,
		                                 4,
		                                 endness=project.arch.memory_endness)
		strncpy_src = state.memory.load(state.regs.esp + 8,
		                                4,
		                                endness=project.arch.memory_endness)
		...
		# destination).
		# (!)
		if state.solver.symbolic(src_contents) and state.solver.symbolic(
		    strncpy_dest):
			...
			# (!)
			password_string = 'TJBPADFJ'.encode()  # :string
			buffer_address = 0x5a505434  # :integer, probably in hexadecimal
	...
	if simulation.found:
		...
		parameter_u = solution_state.solver.eval(stored_solutions[0])
		parameter_20s = solution_state.solver.eval(stored_solutions[1],
		                                           cast_to=bytes).decode()

		solution = f'{parameter_u} {parameter_20s}'
		print(solution)
	else:
