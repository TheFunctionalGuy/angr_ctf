...
def main(argv):
	...
	class ReplacementScanf(angr.SimProcedure):
		...
		def run(self, _, address_u, address_20s):
			...
			for char in scanf1.chop(bits=8):
				...
				# your solution.
				# (!)
				#* Printable ASCII range -1,-1 because space is problematic for scanf
				#* and delete isn't really printable.
	...
	def check_puts(state):
		...
		# (!)
		puts_parameter = state.memory.load(state.regs.esp + 4,
		                                   4,
		                                   endness=project.arch.memory_endness)
		...
		if state.solver.symbolic(puts_parameter):
			...
			# address in .rodata.
			# (!)
			good_job_string_address = 0x42434856 + 5  # :integer, probably hexadecimal
